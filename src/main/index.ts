...
app.whenReady().then(() => {
  ...
  // IPC test
  ipcMain.on('ping', () => {
    console.log(window.isAlwaysOnTop())
  })
  ...
  ipcMain.handle('open-folder', async (event) => {
    ...
    if (!result.canceled) {
      ...
      console.log('[open-folder]', directoryListing)
      return {
        rootDir,
  ...
  // open file
  ipcMain.handle('open-file', async (_, args) => {
    console.log('[open-file]', args)
    const file = fs.readFileSync(args.file?.path, 'utf8')
    return file
  })
